...
import type { FinderAnswers, Game } from "@/lib/types";

type ArrayKey = keyof FinderAnswers;
...
const STEPS: Step[] = [
  {
    key: "platforms",
    question: "Welche Plattform(en) nutzt du?",
    help: "Wähle alle, die für dich infrage kommen.",
    options: PLATFORMS.map((p) => ({ value: p, label: p })),
  },
  {
    key: "genres",
    question: "Welche Genres magst du?",
    help: "Mehrfachauswahl möglich – nimm alles, worauf du Lust hast.",
    options: GENRES.map((g) => ({ value: g, label: g })),
  },
  {
    key: "players",
    question: "Allein oder mit Freunden?",
    help: "Du kannst auch beides auswählen.",
    options: [
  ...
  },
  {
    ...
    help: "Von ganz entspannt bis knallhart – wähle, was passt.",
  ...
  },
  {
    ...
    help: "Such dir eine oder mehrere Stimmungen aus.",
    options: MOODS.map((m) => ({ value: m.value, label: `${m.emoji} ${m.label}` })),
  },
];

export default function GameFinder({ games }: { games: Game[] }) {
  const [step, setStep] = useState(0);
  const [answers, setAnswers] = useState<FinderAnswers>({});
  const [finished, setFinished] = useState(false);

  const results = useMemo(
    () => (finished ? runFinder(answers, 9, games) : []),
    [finished, answers, games],
  );
  ...
  function toggleOption(value: string) {
    setAnswers((prev) => {
      const list = (prev[current.key] as string[] | undefined) ?? [];
  ...
  return (
    <div className="surface mx-auto max-w-2xl p-6 sm:p-8">
      ...
      {selected.length > 0 && (
        <p className="mt-3 text-xs text-accent-2">{selected.length} ausgewählt</p>
      )}
      ...
      <div className="mt-6 flex items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={advance} className="btn btn-ghost btn-sm">
            Überspringen
          </button>
          <button type="button" onClick={advance} className="btn btn-primary btn-sm">
            {step < STEPS.length - 1 ? "Weiter →" : "Ergebnisse zeigen"}
          </button>
        </div>
      </div>
    </div>
